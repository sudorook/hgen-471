...
\begin{align}
P(G_{Mother}=CC|G_{Child}=CC,G_{Father}=CT) &= \frac{P(G_C=CC|G_M=CC,G_F=CT) * P(G_M=CC)}{ \sum_{X \in \{C,T\}} P(G_C=CC|G_M=XC,G_F=CT)} \\
  &= \frac{.5 * .64}{ .5 * .64 + .25 * .32 + 0 * .04} \\
  &= .8
\end{align}

\begin{align}
P(G_{Mother}=CT|G_{Child}=CC,G_{Father}=CT) &= \frac{P(G_C=CC|G_M=CT,G_F=CT) * P(G_M=CC)}{ \sum_{X \in \{C,T\}} P(G_C=CC|G_M=XC,G_F=CT)} \\
  &= \frac{.25 * .16}{ .5 * .64 + .25 * .32 + 0 * .04} \\
  &= .2
\end{align}
...
P(G=CC) &= \sum_{G_M} P(G=CC|G_M) P(G_M) \\
  &= P(G=CC|G_M=CC)P(G_M=CC) + P(G=CC|G_M=CT)P(G_M=CT)\\
  &= .5 * .8 + .25 * .2 = .45
\end{align}

\begin{align}
P(G=CT) &= \sum_{G_M} P(G=CT|G_M) P(G_M) \\
  &= P(G=CT|G_M=CC)P(G_M=CC) + P(G=CT|G_M=CT)P(G_M=CT)\\
  &= .5 * .8 + .5 * .2 = .5
\end{align}

\begin{align}
P(G=TT) &= \sum_{G_M} P(G=TT|G_M) P(G_M) \\
  &= P(G=TT|G_M=CC)P(G_M=CC) + P(G=TT|G_M=CT)P(G_M=CT)\\
  &= 0 * .8 + .25 * .2 = .05
\end{align}
