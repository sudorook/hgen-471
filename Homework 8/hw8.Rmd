...
between individuals. Using a pairwise-difference metric for measuring genetic
variation when structural variation is present will return inflated values
because multiple differences will be attributed to a single mutational
feature/event.
...
relatedness among individuals could confound results. Related individuals are
likely to share alleles, so even some common variants in the broader population
may not be detected. This will deflate genetic variation.
...
genetic diversity that use pairwise base-pair comparisons will overestimate
genetic variation.
...
estimate of the posterior probabilities of nucleotide calls, such as the mean
posterior, against the known distribution at each locus to obtain a set of
locus-specific correction factors. They can then be used as the coefficients to
adjust likelihoods for unknown data. This strategy will require a regression to
be performed at every locus, because the bias for a given sequence in the
genome depends on many sequence specific factors, such as GC content, repeats,
etc. This means that a regression model for a given sequence is likely unusable
for a sequence from a different part of the genome.

To generalize the linear model, one could pick out features that correspond to
high regression coefficients (i.e. high deviation from the expected posterior).
This could be done manually based on known molecular features of DNA or the
sequencing technology, or it could be done in an automated manner by a machine
learning algorithm.
...
\begin{align}
P(CC|D) &\propto P(D|G=CC)P(CC) \\
  &= 8.852928\text{e-}{5} * .999 \\
  &= 8.844075\text{e-}05
\end{align}

\begin{align}
P(CT|D) &\propto P(D|G=CC)P(CC) \\
  &= 0.01384129 * .000\bar{6} \\
  &= 9.227525\text{e-}06
\end{align}

\begin{align}
P(TT|D) &=\propto P(D|G=CC)P(CC)\\
  &= 9.409\text{e-}{9} * .000\bar{3} \\
  &= 3.136333\text{e-}12
\end{align}
...
\begin{align}
P(G_{Mother}=CT|G_{Child}=CT,G_{Father}=CC) &= \frac{P(G_C=CT|G_M=CT,G_F=CC) * P(G_M=CT)}{ \sum_{X \in \{C,T\}} P(G_C=CT|G_M=XT,G_F=CC)P(G_M = XT)} \\
  &= \frac{.5 * .32}{ 0 * .64 + .5 * .32 + 1 * .04} \\
  &= .8
\end{align}

\begin{align}
P(G_{Mother}=TT|G_{Child}=CT,G_{Father}=CC) &= \frac{P(G_C=CT|G_M=TT,G_F=CC) * P(G_M=TT)}{ \sum_{X \in \{C,T\}} P(G_C=CT|G_M=XT,G_F=CT)P(G_M = XT)} \\
