...
can then be used to compute a position-weight matrix for any given sequence
motif, and non-coding regions conserved across samples imply (but do not
necessarily prove) functional significance.
...
classify whether the enhances of interest are active or inactive in diseased
...
effects. The nature of the classes themselves are determined by the modeller -
they can be biophysical (binding affinity for transcription factors, etc.),
sequence-based (likelihoods of genotypes from multiple sequence alignments,
etc.), enrichment of annotations in causal variants, among other things. These
algorithms should also take care to account for linkage, allele frequencies,
and other confounding factors that can produce erroneous classifications. Once
the model faithfully recapitulates the training set, while accounting for risk
of overfitting, it can then be applied to the experimental data to categorize
SNPs into the functional classes, such as the regulatory pathway, whether the
SNP removes/augments function, etc.
